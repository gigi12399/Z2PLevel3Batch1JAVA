...
public class DatabaseService {
	...
	public static Librarian getLogin(String name, String password) {
		Librarian lib = null;
	...
	public static Member searchCardID(String card_id) {
		Member mem = null;
		try (Connection con = MyConnection.getConnection()) {
	...
	public static List<BorrowBook> getAllBorrowBooks() {
		...
		try (Connection con = MyConnection.getConnection()) {
			String query = """
					SELECT o.*, m.name, b.title, l.username
					FROM borrow_books o, members m, books b, librarians l
					WHERE o.card_id = m.card_id AND o.book_id = b.code AND o.librarian_id = l.id
					""";
			PreparedStatement pstm = con.prepareStatement(query);
			ResultSet rs = pstm.executeQuery();
			while (rs.next()) {
	...
	public static List<BorrowBook> searchBorrowBooksByMember(int member_id) {
	...
	public static Book searchBooksByBookID(int book_code) {
		Book book = null;
		try(Connection con = MyConnection.getConnection()) {
			String query = """
					SELECT b.*, a.name 'author_name', c.name 'category_name'
	...
	public static boolean confirmBorrowBooks(List<BorrowBook> borrowBookList) {
		try(Connection con = MyConnection.getConnection()) {
			String query = """
					INSERT INTO borrow_books(card_id, book_id, borrow_date, due_date, librarian_id, fine_fees)
					VALUES(?,?,?,?,?,?)
					""";
			PreparedStatement pstm = con.prepareStatement(query);
			for(var i = 0; i < borrowBookList.size(); i++) {
				BorrowBook bBook = borrowBookList.get(i);
				pstm.setObject(1, bBook.getMember().getCard_id());
				pstm.setObject(2, bBook.getBook().getCode());
				pstm.setDate(3, Date.valueOf(bBook.getBorrow_date()));
				pstm.setDate(4, Date.valueOf(bBook.getDue_date()));
				pstm.setObject(5, bBook.getLibrarian().getId());
				pstm.setInt(6, bBook.getFine_fees());
				pstm.addBatch();
			}
			pstm.executeBatch();
			return true;
	...
	public static void changeAvailable(List<Book> bookList, String is_available) {
		try(Connection con = MyConnection.getConnection()) {
			String query = "UPDATE books SET is_available = ? WHERE code = ?";
			PreparedStatement pstm = con.prepareStatement(query);
			for(var i = 0; i < bookList.size(); i++) {
				pstm.setString(1, is_available);
				pstm.setInt(2, bookList.get(i).getCode());
				pstm.addBatch();
			}
			pstm.executeBatch();
		} catch (Exception e) {
	...
	public static boolean updateReturnBook(BorrowBook bBook) {
		try(Connection con = MyConnection.getConnection()) {
			String query = "UPDATE borrow_books SET return_date = ? , fine_fees = ? WHERE card_id = ? AND book_id = ?";
			PreparedStatement pstm = con.prepareStatement(query);
			pstm.setDate(1, Date.valueOf(bBook.getReturn_date()));
			pstm.setInt(2, bBook.getFine_fees());
			pstm.setInt(3, bBook.getMember().getCard_id());
			pstm.setInt(4, bBook.getBook().getCode());
			pstm.executeUpdate();
			return true;
		} catch (Exception e) {
			e.printStackTrace();
